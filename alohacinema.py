...
plugin = Plugin()
addon = xbmcaddon.Addon("plugin.video.alohacinema")
pluginrootpath = "plugin://plugin.video.alohacinema"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.alohacinema/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.alohacinema(/.+?/).+?\://", item["path"])
			...
			if "spreadsheets/d/" in item["path"]:
				# https://docs.google.com/spreadsheets/d/1EWMWw6B7tI97SSq1RcOcbjmKWk-yicAgoyPaPzs7up0/edit#gid=0
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.alohacinema" in item["path"]:
